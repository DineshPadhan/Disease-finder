...
def helper(dis):
  ...
  med = ast.literal_eval(med[0]) if len(med) and isinstance(med[0], str) else list(med)

  die = diets[diets['Disease'] == dis]['Diet'].values
  die = ast.literal_eval(die[0]) if len(die) and isinstance(die[0], str) else list(die)
...
def predict():
  if request.method == 'POST':
    ...
    if not user_symptoms:
      return render_template('index.html', symptoms_list=list(symptoms_dict.keys()), warning_message='Please add at least one symptom before prediction.')

    invalid_symptoms = [sym for sym in user_symptoms if sym not in symptoms_dict]
    if invalid_symptoms:
      return render_template('index.html', symptoms_list=list(symptoms_dict.keys()), warning_message='Please select valid symptoms from suggestions.', user_symptoms=','.join(user_symptoms))

    predicted_disease = get_predicted_values(user_symptoms)
    descr, pre, med, die, wrkout = helper(predicted_disease)

    return render_template('index.html', user_symptoms=symptoms, predicted_disease=predicted_disease, dis_des=descr, dis_pre=pre, dis_med=med, dis_diet=die, dis_wrkout=wrkout, symptoms_list=list(symptoms_dict.keys()))
